...
export default function UserRoadmapPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (response.data.success) setBoards(response.data.data.boards);
      ...
      } catch { console.error('Error initializing roadmap'); }
  ...
  useEffect(() => {
    if (initialized.current) fetchBoardData(selectedBoard);
  }, [selectedBoard]);
                            ...
                            <div onClick={(e) => { e.stopPropagation(); toggleVote(post.id); }}
                              className={`inline-flex flex-col items-center justify-center h-10 rounded-lg border font-bold shrink-0 cursor-pointer transition-all ${
                                votes[post.id]?.voted
                                  ...
                                  : (d ? 'border-gray-600 text-gray-300 hover:border-gray-400' : 'border-gray-200 text-gray-700 hover:border-gray-400')
