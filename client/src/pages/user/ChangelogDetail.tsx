...
export default function UserChangelogDetail() {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setEntry((prev) =>
          prev
            ? { ...prev, isLiked: response.data.data.liked, _count: { likes: response.data.data.likeCount } }
            : null
        );
      }
    } catch (error) {
  ...
  if (!entry) {
    return (
      <UserLayout>
        <div className="text-center py-12">Entry not found</div>
      </UserLayout>
    );
  }
  ...
  return (
    <UserLayout>
      <div>
        {/* Header */}
        <div className={`border-b pb-6 mb-8 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={() => navigate('/user/changelog')}
