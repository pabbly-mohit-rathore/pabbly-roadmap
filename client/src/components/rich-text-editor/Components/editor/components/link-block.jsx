...
export function LinkBlock({ editor, rawMode = false, rawApi }) {
  ...
  const handleOpenPopover = (event) => {
    ...
    if (!rawMode && editor) {
      // Check for text link first
      previousUrl = editor?.getAttributes('link').href;
      
      // If no text link, check if image is selected and has href
      if (!previousUrl) {
        try {
          const { state } = editor;
          ...
          state.doc.nodesBetween(from, to, (node) => {
            if (node.type.name === 'image' && node.attrs.href) {
              previousUrl = node.attrs.href;
            }
          });
        } catch (e) {
          // Ignore errors
    ...
    const update = () => {
      if (rawMode) {
        try {
          ...
          const selectedText = (r.toString ? r.toString() : '').trim();
          
          // Link button enabled jab text selected ho
          setCanLink(!r.collapsed && selectedText.length > 0);
          
          // Unlink button enabled jab caret link ke andar ho
          const node = sel.anchorNode instanceof Element ? sel.anchorNode : sel.anchorNode?.parentElement;
          const a = node && node.closest ? node.closest('a') : null;
          setCanUnlink(Boolean(a));
        } catch (e) {
          setCanLink(false);
          setCanUnlink(false);
        }
      } else {
        // Text editor mode - check for text selection or image selection
        try {
          ...
          // Enable link button if there's text selected OR an image is selected
          const hasTextSelection = selection && !selection.empty;
          setCanLink(hasTextSelection || hasImage);
          
          // Enable unlink if link mark is active OR image has href attribute
          ...
          setCanUnlink(editor?.isActive('link') || imageHasLink);
        } catch (e) {
          setCanLink(false);
          setCanUnlink(false);
        }
  ...
  const handleUpdateUrl = useCallback(() => {
    ...
    if (rawMode) {
      ...
      if (!url) {
        rawApi?.unsetLinkAtSelection?.();
      } else {
        rawApi?.wrapSelectionWithLink?.(url);
      }
      return;
    }
    ...
    // Find if there's an image node in the selection
    let imageNode = null;
    let imagePos = null;
    ...
    // If an image is selected, update its link attributes
    if (imageNode && imagePos !== null) {
      if (!url || url.trim() === '') {
        // Remove link: update image attributes to remove href and target
        editor
          .chain()
          .focus()
        ...
        // Add/update link: update image attributes to include href and target
        const finalUrl = url.trim();
        editor
          .chain()
          .focus()
          .setTextSelection({ from: imagePos, to: imagePos + imageNode.nodeSize })
          .updateAttributes('image', { href: finalUrl, target: '_blank' })
          .run();
      }
      return;
    }
  
    // Text editor logic for text selection
    if (!url || url.trim() === '') {
      editor?.chain().focus().extendMarkRange('link').unsetLink().run();
    } else {
      editor?.chain().focus().extendMarkRange('link').setLink({ href: url.trim() }).run();
    }
  }, [editor, url, rawMode, rawApi, handleClosePopover]);
  ...
  return (
    <>
      <ToolbarItem
        aria-label="Link"
        active={rawMode ? false : editor?.isActive('link')}
